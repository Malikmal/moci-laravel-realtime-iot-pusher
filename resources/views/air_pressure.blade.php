...
<div class="container mt-xl-50 mt-sm-30 mt-15">
    ...
    <div class="hk-pg-header align-items-top">
        <div>
			<h2 class="hk-pg-title font-weight-600 mb-10">Air Pressure Analytics</h2>
		...
		<div class="d-flex w-250p">
			...
			<select class="form-control custom-select custom-select-sm mr-15" onchange="location=this.value">
                @foreach ($device as $item)
                    ...
                    <option
                        value="{{route('airPressure', $item->id)}} "
                        @if ($item->id == $id)
                            selected
                        @endif
                    >{{$item->place}}</option>
					...
					<div class="card-group hk-dash-type-2">
						<div class="card card-sm">
							<div class="card-body">
								<div class="d-flex justify-content-between mb-5">
									<div>
										<span class="d-block font-15 text-dark font-weight-500">Current Air Pressure</span>
									</div>
									<div>
								...
								<div>
									<span class="d-block display-4 text-dark mb-5" id="current_airPressure">34 mBar</span>
						...
						<div class="card card-sm">
							<div class="card-body">
								...
								<div>
									<span class="d-block display-4 text-dark mb-5">{{$thisday['morning']->airPressure ?? ''}} mBar</span>
						...
						<div class="card card-sm">
							<div class="card-body">
								...
								<div>
									<span class="d-block display-4 text-dark mb-5">{{$thisday['afternoon']->airPressure ?? ''}} mBar</span>
						...
						<div class="card card-sm">
							<div class="card-body">
								...
								<div>
									<span class="d-block display-4 text-dark mb-5">{{$thisday['evening']->airPressure ?? ''}} mBar</span>
								...
								<div class="table-responsive">
									<table class="table table-sm table-hover mb-0">
										<thead>
											<tr>
												<th>Day</th>
												<th class="w-30">Avg (mBar)</th>
												<th class="w-35">min (mBar)</th>
												<th class="w-35">Max (mBar)</th>
												<th>Graph</th>
											</tr>
										</thead>
										<tbody>
                                            @foreach ($daily as $item)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="progress-wrap lb-side-left mnw-75p">
                                                            <div class="progress-lb-wrap">
                                                                <label class="progress-label mnw-50p">{{round($item->airPressure, 2)}}</label>
                                                    ...
                                                    <td>{{round($item->airPressure, 2)}}</td>
                                                    <td>{{round($item->airPressure, 2)}}</td>
...
@push('script')
    ...
    {{-- <script src=" {{ asset('template/dist/js/dashboard-data.js') }}">s</script> --}}
  ...
  //           }
  //           document.getElementById('current_airPressure').innerHTML = current + "° C";
  ...
  //   }, /*40*/100000);
	...
	@foreach($sensor as $item)
		// console.log({{$item->airPressure}});
		sensors[{{ count($sensor) - $loop->iteration}} - 1] = [{{ count($sensor) - $loop->iteration}} - 1, {{$item->airPressure}}]
	@endforeach

	// @for($i = count($sensor) - 1; $i >= 0 ; $i--)

	// 	sensors[$i] =  [{{$i}}, {{$sensor[$i]->airPressure}}];
    ...
    channel.bind('my-event', function(data) {
        if(data.id == {{$id}})
        {
            ...
            var last_temp = parseInt(current_airPressure_id.textContent);
            current_airPressure_id.innerHTML = data.airPressure + " mBar";

            //current percentage
            var percentage = (data.airPressure - last_temp) * 100 / last_temp;
            var isPlus ;
            var percentageDiv = document.getElementById('current_percentage');
            if(percentage >= 0)
            {
            ...
            // this.sensors.push([this.sensors.length, parseInt(data.airPressure)]);
            ...
            // this.sensors.push([this.sensors.length, parseInt(data.airPressure)]);
            // dataGraph.push(this.sensors.length, data.airPressure);
            var fill_line_chart_data = {
                data : newSensors
            }
        ...
        if( $('#sparkline_1').length > 0 ){
            ...
            @foreach($daily as $item)
                ...
                @foreach($item->collect as $item2)
                    <?php $j++; ?>
                    airPressure[{{$j - 1}}] = {{round($item2->airPressure, 2)}};
                    @if($j > 8)
                        @break
                    @endif
                @endforeach
                $("#sparkline_{{$i}}").sparkline(airPressure, {
                    type: 'line',
                    width: '100',
                    height: '34',
                    resize: true,
