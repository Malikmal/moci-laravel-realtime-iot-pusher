...
<div class="container mt-xl-50 mt-sm-30 mt-15">
    ...
    <div class="hk-pg-header align-items-top">
        <div>
			<h2 class="hk-pg-title font-weight-600 mb-10">Rain Analytics</h2>
					...
					<div class="card-group hk-dash-type-2">
						<div class="card card-sm">
							<div class="card-body">
								<div class="d-flex justify-content-between mb-5">
									<div>
										<span class="d-block font-15 text-dark font-weight-500">Current Rain Percentage</span>
									</div>
									<div>
								...
								<div>
									<span class="d-block display-4 text-dark mb-5" id="current_rain">34 %</span>
						...
						<div class="card card-sm">
							<div class="card-body">
								...
								<div>
									<span class="d-block display-4 text-dark mb-5">{{$thisday['morning']->rain ?? ''}} %</span>
						...
						<div class="card card-sm">
							<div class="card-body">
								...
								<div>
									<span class="d-block display-4 text-dark mb-5">{{$thisday['afternoon']->rain ?? ''}} %</span>
						...
						<div class="card card-sm">
							<div class="card-body">
								...
								<div>
									<span class="d-block display-4 text-dark mb-5">{{$thisday['evening']->rain ?? ''}} %</span>
										...
										<tbody>
                                            @foreach ($daily as $item)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="progress-wrap lb-side-left mnw-75p">
                                                            <div class="progress-lb-wrap">
                                                                <label class="progress-label mnw-50p">{{round($item->rain, 2)}} %</label>
                                                    ...
                                                    <td>{{round($item->rainMin, 2)}} %</td>
                                                    <td>{{round($item->rainMax, 2)}} %</td>
...
@push('script')
  ...
  //           }
  //           document.getElementById('current_rain').innerHTML = current + "° C";
  ...
  //   }, /*40*/100000);
	...
	@foreach($sensor as $item)
		...
		sensors[{{ count($sensor) - $loop->iteration}} - 1] = [{{ count($sensor) - $loop->iteration}} - 1, {{$item->rain}}]
	@endforeach

	// @for($i = count($sensor) - 1; $i >= 0 ; $i--)

	// 	sensors[$i] =  [{{$i}}, {{$sensor[$i]->rain}}];
    ...
    channel.bind('my-event', function(data) {
        if(data.id == {{$id}})
        {
            ...
            //current rain 	//and 		//parsing value of current rain
            var current_rain_id = document.getElementById('current_rain');
            var last_temp = parseInt(current_rain_id.textContent);
            current_rain_id.innerHTML = data.rain + "° C";

            //current percentage
            var percentage = (data.rain - last_temp) * 100 / last_temp;
            var isPlus ;
            var percentageDiv = document.getElementById('current_percentage');
            if(percentage >= 0)
            {
            ...
            // }
            // console.log(this.sensors);
            this.sensors.push([this.sensors.length, parseInt(data.rain)]);
            var newSensors = [];
        ...
        if( $('#sparkline_1').length > 0 ){
            ...
            @foreach($daily as $item)
                ...
                @foreach($item->collect as $item2)
                    <?php $j++; ?>
                    rain[{{$j - 1}}] = {{round($item2->rain, 2)}};
                    @if($j > 8)
                        @break
                    @endif
                @endforeach
                $("#sparkline_{{$i}}").sparkline(rain, {
                    type: 'line',
                    width: '100',
                    height: '34',
                    resize: true,
